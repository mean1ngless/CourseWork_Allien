...
class ConsoleInterface:
    ...
    def problem_size_experiment(self):
        ...
        try:
            ...
            plt.subplot(2, 2, 1)
            plt.plot(sizes, greedy_diffs, 'b-', label='Жадібний')
            plt.plot(sizes, genetic_diffs, 'r-', label='Генетичний')
            plt.xlabel('Кількість предметів')
            plt.ylabel('Добуток різниць')
            plt.title('Якість розподілу')
            plt.grid(True)
            plt.legend()

            plt.subplot(2, 2, 2)
            plt.plot(sizes, greedy_times, 'b-o')
            plt.xlabel('Кількість предметів')
            plt.ylabel('Час (сек)')
            plt.title('Час жадібного алгоритму')
            plt.grid(True)

            plt.subplot(2, 2, 3)
            plt.plot(sizes, genetic_times, 'r-o')
            plt.xlabel('Кількість предметів')
            plt.ylabel('Час (сек)')
            plt.title('Час генетичного алгоритму')
            plt.grid(True)

            plt.subplot(2, 2, 4)
            plt.plot(sizes, greedy_times, 'b--', label='Жадібний')
            plt.plot(sizes, genetic_times, 'r--', label='Генетичний')
            plt.xlabel('Кількість предметів')
            plt.ylabel('Час (сек)')
